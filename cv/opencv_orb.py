...
def filter_matches_1(kpts1, kpts2, matches, ratio = 0.75):
    "当最近的2个点的距离比小于0.75时，认为匹配有效"
    mkpts1, mkpts2,good_matches = [], [],[]
    for m in matches:
        if len(m) == 3 and m[0].distance < m[1].distance * ratio:
            m = m[0]
...
def filter_matches_2(matches,threhold=30):
    "当匹配点距离小于最小距离的2倍或threhold时，认为匹配有效"
...
############匹配方法1#################
match_1=matcher.knnMatch(desc1, trainDescriptors = desc2, k = 2) # KNN匹配最近的2个点
p1, p2, kp_pairs,good_matches= filter_matches_1(kpts1, kpts2, match_1)
#############匹配方法2################
match_2=matcher.match(desc1,desc2,None) #匹配最近的点
min_dist,max_dist,good_matches=filter_matches_2(match_2)

img_goodmatch=cv2.drawMatches(img1,kpts1,img2,kpts2,good_matches,None)   #绘制正确的匹配点

cv2.imshow('img-match', img_goodmatch)
cv2.waitKey()
